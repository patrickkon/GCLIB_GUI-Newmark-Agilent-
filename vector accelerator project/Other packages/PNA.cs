    ...
    class PNA
    {
        ...
        public void SetupMeasurement()
        {
            if (this.online)
            {
                if (calFile != "")
                {
                    application.Recall(calFile);
                    //guid = calSet.GetGUID;
                    //channel = application.ActiveChannel;
                    channel = application.Channel(2); //make the argument 1 for S11, 2 for S21, or whatever the calset says
                    //channel.SelectCalSet(guid, true);
                }
                else
                {
                    channel = application.Channel(2); //default channel when no calset selected
                    //channel.application.ActiveChannel;
                    ...
                    //channel.StartPower = -80; //increasing port power doesnt really help
                    //channel.StopPower = -80; //increasing port power doesnt really help
                }
                //application.CreateMeasurement(1, type.ToString(), 1, 1); //type.ToString is e.g. "S21" - whatever selected by user
